...
def save_preprepared_data(resource:str, train_scaled, test_scaled, val_scaled, scaler, column_names, prints:bool = False):
    ...
    file_path = os.path.join(output_dir, f"{resource}_data.npz")
...
def load_preprepared_data(resource:str, prints:bool = False):
    file_path = os.path.join(os.getcwd(), "train_values", f"{resource}_data.npz")

    if not os.path.exists(file_path):
        raise FileNotFoundError(f"Prepared data file not found at {file_path}")
    ...
    if prints:
        print(f"Loaded prepared data from {file_path}")
        print(f"  - Train shape: {train_scaled.shape}")
        print(f"  - Test shape: {test_scaled.shape}")
        print(f"  - Validation shape: {val_scaled.shape}")
        print(f"  - Columns: {len(column_names)}\n")
